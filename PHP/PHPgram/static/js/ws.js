//접속 시도
const ws = new WebSocket("ws://192.168.0.19:8090/");
...
(function() {
    ...
    if(gData && gData.dataset.loginiuser) {
        ...
        ws.onmessage = function(e) {
            ...
            console.log(json);   
            const unreadDm = document.querySelector(`#unreadDm${json.iuser}`);
                ...
                case 'dm':                        
                    ...
                    if(inputIdm) {  //현재 페이지가 dm페이지라면
                        if(parseInt(inputIdm.value) === json.idm) { //현재 페이지가 dm페이지면서 해당 사람과 대화중이라면
                            ...
                            //대화중이 아닐경우에 구현하는 부분(사람이름 옆에 숫자가 나오게)
                            const count = unreadDm.querySelector(`#unreadCount${json.iuser}`);
                            count.innerHTML = parseInt(count.innerText) + 1;
                            unreadDm.classList.remove("d-none");
                        }
